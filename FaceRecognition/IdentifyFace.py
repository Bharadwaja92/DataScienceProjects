...
def prepare_training_data(data_folder_path):
    ...
    for dir_name in dirs:
        ...
        for image_name in subject_images_names:
            ...
            if face is not None:
                print('face is', face.shape)
                cv2.imwrite('face.jpg', face)
                f2 = cv2.imread('face.jpg')
                f2 = cv2.cvtColor(f2, cv2.COLOR_BGR2GRAY)
                face = cv2.resize(f2, (100, 125))
                print('face is', face.shape)
                # # f1 = cv2.cvtColor(np.array(scipy.misc.toimage(face)), cv2.COLOR_RGB2BGR)
                # f1 = cv2.cvtColor(face, cv2.COLOR_RGB2BGR)
                # cv2.imshow('face', f1)
                # cv2.waitKey(10)
                faces.append(face)
                labels.append(label)
...
test_img5 = cv2.imread("/home/saibharadwaj/Downloads/test5_1.jpeg")          # Sushmita Sen
...
test_img7 = cv2.imread("/home/saibharadwaj/Downloads/testDilse4.png")          # Dilse
predicted_img7 = predict(test_img7)
cv2.imshow('Predicted9', predicted_img7)

test_img7 = cv2.imread("/home/saibharadwaj/Downloads/testDilse1.png")          # Shahrukh khan Dilse
predicted_img7 = predict(test_img7)
cv2.imshow('Predicted8', predicted_img7)
