...
What type of graph would work best to display the proportion of team members who own a cat, dog, bird, or no pet? 
(Assume each team member can only own one pet.)
Pie


Which line of code will create a bar graph with error bars displaying +/- 4 error, with caps of size 10?
plt.bar(x, y, yerr=4, capsize=10)
...
The lower-bound y-values to plot
...
The pie chart will now display percentages, rounded to the nearest int, on each slice of the pie.


What is the command to set x-axis ticks to be "Carbohydrates", "Lipids", "Protein"?
ax.set_xticklabels(["Carbohydrates", "Lipids", "Protein"])
